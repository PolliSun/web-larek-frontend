...
1. `(Model)`: Хранит данные, такие как информация о карточках товаров, состояние корзины, информация о заказе и другие данные.
Классы:
- Model,
- AppState,
- Order и Contact

2. `(View)`: Отображает данные для пользователя и обрабатывает действия пользователя, такие как нажатия кнопок, заполнение форм и другие действия.
Классы:
- Component<T>,
- Card,
- Basket,
...
3. `(Presenter)`: Выполняет бизнес-логику, такую как добавление товаров в корзину, оформление заказа и обработка платежей.
Классы:
- Api,
- EventEmitter,
- CardsApi
...
### 2. IInfoForm
Этот интерфейс содержит все свойства, определенные в интерфейсах IPaymentForm и IContactsForm, а именно
...
order: IInfoForm | null - Информацию о текущем заказе или null. Содержит свойства для информации о платеже (payment, address) и контактных данных (email, phone).
...
- order: Объект, содержащий информацию о заказе, такую как адрес доставки, способ оплаты и контактные данные.
- formErrors: Объект с информацией об ошибках валидации форм заказа.

### Методы:

- `getOrderData`(): Этот метод возвращает данные заказа в формате IOrder. Включая информацию о товарах, адресе доставки, контактных данных покупателя и общей стоимости заказа. Информация о товарах содержится в виде массива идентификаторов (items) типа string[], в котором перечислены идентификаторы товаров, находящихся в корзине. Общая стоимость заказа указана в виде числа (total).
- `setCatalog(items: ICard[])`: Этот метод устанавливает каталог товаров. Принимает массив объектов карточек товаров (items). После обновления каталога метод вызывает событие items:changed, чтобы уведомить другие компоненты о том, что каталог был изменен, и передает обновленный каталог вместе с этим событием.
...
- `reloadBasket`(): Этот метод перезагружает состояние корзины. Он вызывается после изменений в корзине и отправляет события об этих изменениях.
- `clearBasket`(): Этот метод очищает корзину. Устанавливает пустой массив для корзины и обновляет ее состояние.
- `clearOrder`(): Этот метод очищает данные в формах оплаты и контактов. Устанавливает пустые значения и обновляет состояние заказа.
- `validateAddressForm`(): Этот метод проверяет форму адреса и оплаты на валидность. Проверяет наличие адреса доставки и выбранного способа оплаты. Если есть ошибки, устанавливает их и оповещает об изменении ошибок.
